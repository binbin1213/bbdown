...
public static partial class BBDownUtil
{
    ...
    public static string? FindExecutable(string name)
    {
        ...
        var osFolder = OperatingSystem.IsWindows() ? "win" : (OperatingSystem.IsMacOS() ? "osx" : "linux");
        var searchPath = new []
        {
            Environment.CurrentDirectory,
            Program.APP_DIR,
            Path.Combine(Environment.CurrentDirectory, "tools"),
            Path.Combine(Environment.CurrentDirectory, "tools", osFolder)
        };
    ...
    public static async Task<bool> CheckLogin(string cookie)
    {
        try
        {
            ...
            var imgUrlStr = wbi_img.GetProperty("img_url").GetString();
            var subUrlStr = wbi_img.GetProperty("sub_url").GetString();
            if (!string.IsNullOrEmpty(imgUrlStr) && !string.IsNullOrEmpty(subUrlStr))
            {
                Core.Config.WBI = GetMixinKey(RSubString(imgUrlStr) + RSubString(subUrlStr));
            }
